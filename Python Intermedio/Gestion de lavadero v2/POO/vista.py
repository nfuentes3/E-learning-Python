...
db = OperacionesDB() #Genero instancia de clase
...
#Boton alta
boton_alta = Button(master, text="Alta", command=lambda:CrudVista.alta())
...
boton_consultar = Button(master, text="Buscar", command=lambda:CrudVista.consulta(tree))
...
boton_calcular = Button(master, text="Calcular", command=lambda:CrudVista.balance_total())
...
#Boton cambiar color
boton_color = Button(master, text="Color fondo")
...
#Boton modificar
boton_modificar = Button(master, text="Modificar", command=lambda:CrudVista.modificar())
boton_modificar.grid(row=8, column=0, pady=10, padx=50, ipadx=10,sticky="w")

#Boton borrar
boton_borrar = Button(master, text="Borrar", command=lambda:CrudVista.borrar())
boton_borrar.grid(row=8, column=3, ipadx=10)

#Boton ver todos
boton_todos = Button(master, text="Ver todos", command=lambda:AccionesVista.actualizar_tree(tree))
...
class CrudVista:
    ...
    def alta():
        if v_telefono.get() != '' and v_telefono.get() != int:
            db.alta_orden(v_nombre.get(), v_telefono.get(), v_tipo.get(), v_cantidad.get(), v_fecha_entrega.get(), v_precio.get())
            AccionesVista.actualizar_tree(tree)
            AccionesVista.limpiar_entrys()
            ...
            mensaje_error = Label(master, text="Error! El campo 'Telefono' no debe estar vacio y solo debe contenter numeros!", fg="red")
            mensaje_error.place(x=200, y=655)
    ...
    def consulta(tree):
        try:
            if v_consulta.get() == '':
                mensaje_borrar = Label(master, text="El campo de busqueda no debe estar vacio!", fg="blue")
                mensaje_borrar.place(x=290, y=655)
                master.after(6000, lambda label:label.destroy(), mensaje_borrar)
            else:
                ...
                for orden in ordenes:
                    tree.delete(orden)
                nombre = v_consulta.get()
                resultado = db.consulta_nombre(nombre)
    ...
    def modificar():
        try:
            ...
            orden = tree.item(seleccion)
            id_orden = orden['text']
            if v_nombre == '' or v_telefono == '' or v_tipo == '' or v_cantidad == '' or v_fecha_entrega == '' or v_precio == '' :
                ...
                db.modificar_orden(id_orden, v_nombre.get(),v_telefono.get(), v_tipo.get(), v_cantidad.get(),v_fecha_entrega.get(),v_precio.get())
            ...
            print("ERROR: No se pudo modificar la orden (vista)")
    ...
    def balance_total():
        ...
        for fila in resultado:
            precio_total += fila[6]
        print(f"El balance total por cobrar es de: ${precio_total}")
        print("***" * 10)
